...
int NameingConstructMapper(SAtuple* arr, int* NodesBucketSize, int globalLen, mapperDuple* mapper12Dummy,int rank,int size,bool verbose,int pDC3Level, mapperDuple** MapperfromStuple)
{	
	/// calculate locallen for each processor 
	...
	/// Send naming info to all nodes to check if there is any duplicates
	MPI_Allreduce (&SendCount,&ReceiveCount,1,MPI_INT,MPI_SUM,MPI_COMM_WORLD);

	/// If no duplicates occur
	...
	else 
	{
		...
		int recvLen= all_sort_Mapper_recursion(mapper12Dummy, &SortedMapper, localLen,rank, size,pDC3Level);		
		...
		/// We use it for start ranking Mappers. 
