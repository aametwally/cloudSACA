...
int all_sort_psrs_i(SAtuple *A, int A_size, SAtuple **B,int size, int rank) 
{
	...
	/// Assign the pointer of B to the address of recv_buf
	B_size = trecv;
...
{
	//~ printf("thread=%d, pDC3Level=%d, mapperSize=%d, all_sort_Mapper_recursion\n",rank,pDC3Level,mapperSize);
	int i, recvLen;
    int *pivots,     ///array of values of indices of the pivots
        *partitions, ///array of positions of the pivots in the mapper duple array
        *recv_cnt,   /// number of keys to receive from each PN 
	...
	/// Local Sort 
	fastsort_DC3(recv_buf,recvLen);

	
	*SortedMapper=recv_buf;
