...
int pDC3(int* text,int textLenLocal, int textLenGlobal,int rank, int size,bool verbose, STuple** S_Tuple_Sorted, int pDC3Level)
{
	...
	StartTime=clock();
	int tuples12SortedLenLocal= all_sort_psrs_i(tuples12, tuples12lenLocal, &tuples12Sorted, size, rank);
	...
	if (verbose==1 &&rank==0)
	{	
		elapsedTime(rank, StartTime,"PSRS Tuples12",pDC3Level);
	...
	{	
		...
		printf("Memory After PSRS S, pDC3Level=%d \n",pDC3Level);
		memory_usage(rank);
	}
