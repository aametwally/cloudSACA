...
int all_sort_psrs_i(SAtuple *A, int A_size, SAtuple **B,int size, int rank, int pDC3Level) 
{
	int i, B_size;
	...
	/// Gather all samples into the Master node to sort then then broadcast the selected pivot
	...
	{
		if(i!=size-1) 
		send_cnt[i] = max(0, prtbound[i+1] - prtbound[i]); 
		else
		send_cnt[i] = max(0, prtbound[i+1] - prtbound[i]+1); 
	}
	...
	/// Local sort for the tuples	
	MPI_Barrier(MPI_COMM_WORLD);
	StartTime=clock();

	SAtuple* result; //= (STuple *)malloc(trecv*sizeof(STuple));
...
int all_sort_Mapper_i(mapperDuple* mapper, int mapperSize, int localTextLen, int globalTextLen,int rank, int size,int pDC3Level,int** rankArray)
{
	...
	//~ for(i=0; i<mapperSize; i++) 
	//~ printf("thread=%d, UNSortedMapper[%d]=(%d, %d)\n", rank,i,mapper[i].rank, mapper[i].index);
...
int  all_sort_Mapper_recursion(mapperDuple* mapper, mapperDuple **SortedMapper, int mapperSize, int rank, int size,int pDC3Level)
{
	...
	MPI_Barrier(MPI_COMM_WORLD);
	int StartTime=clock();
	fastsort_DC3(mapper,mapperSize);
	MPI_Barrier(MPI_COMM_WORLD);
	if (rank==0)
...
int all_sort_S_i(STuple *A,int A_size, STuple **B,int size, int rank,int pDC3Level)
{
	...
	//  trecv = recv_off[size-1] + recv_cnt[size-1];
