...
main(int argc, char *argv[]) 
{
    ...
    ///STEP #1: Open the sequence file for reading
   	...
   	if (verbose==1 &&rank==0)
   	{
		...
		memory_usage(rank,"Memory After Sequence read", pDC3Level);
	}
	...
	/// gather the partial SAs from all noes to the master node
	...
	if (rank==0)
	{
		...
		if (review==1)
		CheckCorrectness(reference,OutputFile);
	}
	free(SAFinal);
	MPI_Finalize ();
}
