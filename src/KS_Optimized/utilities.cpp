...
void memory_usage(int rank, string label, int pDC3Level) 
{
	...
	string s;
	int start=0;
	printf("Thread=%d, %s= ", rank, label.c_str());
	...
	{		
		if(s.substr(0, 5) == "VmHWM") {
				...
				printf("%s, pDC3Level=%d\n",(s.substr(start, end-start+2)).c_str(), pDC3Level);
		}
	}
}


/// Save The Output SA to an external file 
...
/// How to use KS Algorithm, this message pops up when the user enters wrong command line
