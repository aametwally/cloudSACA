...
certificate_path = '/media/ahmed/1578-4B0E/WindowsAzure/AzureCert/mycert.pem'
pkfile = "/media/ahmed/1578-4B0E/WindowsAzure/AzureCert/mycert.cer"
...
def myupload(blob_service, container_name, blob_name, file_path):
    blob_service.create_container(container_name, None, None, False)
    blob_service.put_blob(container_name, blob_name, '', 'PageBlob',x_ms_blob_content_length=9216)
    data_sent=0
    sent = 0
    block_ids = []
    index = 0
    with open(file_path, 'rb') as f:
        while True:
            ...
            if data:
                ...
                data_sent += pageSize
                sent = data_sent/(1024*1024)
                sys.stdout.write("\rUploaded data = "+str(sent)+" MB")
                sys.stdout.flush()
            else:
                break
    ...
    with open(file_path, 'rb') as f:
        while True:
            ...
            if data:
                length = len(data)
                #block_id = base64.b64encode(str(index))
                x_range = str(index)+"-"+str(end_index)
                #x_range = base64.b64encode(x_range)
                blob_service.put_page(container_name, blob_name, data, x_range,'update')
                #block_ids.append(block_id)
                index += 512
                end_index +=512
                data_sent += pageSize
...
container_name 	= "ahmedblob2"
blob_name		= "read.pdf"
file_path  = "/home/ahmed/Desktop/upload.py"
upload_block_blob(blob_service,container_name,blob_name,file_path)
...
#createInstances(sms,"elasticahmed10","West US","b39f27a8b8c64d52b05eac6a62ebad85__Ubuntu-12_04_2-LTS-amd64-server-20130325-en-us-30GB","Small",pkfile,2)
